...
myLayout = avoidStruts (tiled ||| mtiled ||| Full)
  where
     ...
     -- tiled   = gaps [(U,10), (R,10), (D,10), (L,10)] $ Tall nmaster delta ratio
     tiled   = spacingRaw False (Border 0 15 0 15) True (Border 15 0 15 0) True $ Tall nmaster delta ratio
     mtiled  = Mirror tiled 
...
myLogHook xmproc = dynamicLogWithPP $ namedScratchpadFilterOutWorkspacePP $ xmobarPP
	...
	, ppCurrent         = xmobarColor "#fabd2f" "" . wrap "<box type=Bottom width=2 mb=2 color=#fabd2f>" "</box>"
	, ppVisible         = xmobarColor "#ffffff" "" -- . clickable
	, ppHidden          = xmobarColor "#ffffff" "" . wrap "<box type=Top width=2 mt=2 color=#82aaff>" "</box>" -- . clickable
